...
class Food:
    ...
    def repawn(self, snake_body):
        self.position = self.randoms_pos(snake_body)
...
def draw_grid(surface):
    ...
    for y in range(0,HEIGHT, CELL):
        pygame.drwa.line(surface, GRID_COLOR, (0,y),(WIDTH, y))
...
def draw_text_center(surface, text, size, y_offset=0):
    front = pygame.font.Sysfront("consolas", size, bold=True)
    text_surf = front.render(center=(WIDTH // 2, HEIGHT // 2 +  y_offset))
    rect = text_surf.get_rect(center=(WIDTH // 2, HEIGHT // 2 + y_offset))
    surface.bilit(text_surf, rect)
